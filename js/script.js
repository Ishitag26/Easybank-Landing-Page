...
//media query list when window size matches the (min-width:1000px)
const mquery = window.matchMedia("(min-width: 1000px)");

//remove the mobile class from  navlist on desktop mode(when the window is wider than 1000px) and set the toggle image back to the hamburger icon
mquery.addListener(() =>{
    if(mquery.matches){
